...
def vin_main_fn(img):
    ...
    #Detect VIN Region
    #os.system('python yolov5/detect.py --weights yolov5/yolov5/weights/large/paper/best.pt --img 416 --conf 0.6 --source temp/gray_image.jpg --project runs/detect/ --name exp --save-crop')
...
#
    ...
    if isdir == False:
        txts_vin,score_vin=vin_ocr_model(img)   
    else:
        print('Dir exist')
        ...
        #Pass images to OCR
        txts_vin,score_vin=vin_ocr_model("runs/temp/inverted.jpg")
    torch.cuda.empty_cache()
    
    return txts_vin,score_vin
...
def fake_image_detector_two(img_in):
    ...
    pred3=tmp_df.dd
    if float(pred3.__round__(2)) < 0.60:
        image_remarks="Bad image quality:Please upload new image"
    elif float(pred3.__round__(2)) >= 0.60 and pred3.__round__(2) <=80:
        image_remarks="Image contains more noice:"
    elif float(pred3.__round__(2)) > 0.90:
        image_remarks="Image Quality: OK" 
    return result2,pred3.__round__(2),image_remarks
########################################
